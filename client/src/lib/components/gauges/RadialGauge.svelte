<script lang="ts">
  ...
  const percentage = $derived((maxValue !== minValue) ? Math.min(100, Math.max(0, ((value - minValue) / (maxValue - minValue)) * 100)) : null);
  ...
  const validGauge = $derived(isValidNumber(size) && size > 0 && isValidNumber(radius) && radius > 0 && isValidNumber(center));

  // Calculate arc path
  const arcLength = $derived(endAngle - startAngle);
  const progressAngle = $derived(startAngle + (arcLength * (percentage ?? 0) / 100));

  const validAngles = $derived(isValidNumber(startAngle) && isValidNumber(endAngle) && isValidNumber(progressAngle));
  const validArc = $derived(validGauge && validAngles && isValidNumber(arcLength) && isValidNumber(percentage));

  const backgroundArc = $derived(validArc ? describeArc(center, center, radius, startAngle, endAngle) : '');
  const progressArc = $derived(validArc ? describeArc(center, center, radius, startAngle, progressAngle) : '');
